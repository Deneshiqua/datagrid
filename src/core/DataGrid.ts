...
export class DataGrid implements DataGridInstance {
  ...
  getGroupState(): { columnId: string; direction: 'asc' | 'desc' } | null {
  ...
  expandAllGroups(): void {
    ...
    data.forEach(row => {
      if (row['_isGroupHeader']) {
        this.dataManager.expandGroup(row['_groupKey'] as string);
      }
    });
    this.expandedGroups = this.dataManager.getExpandedGroups();
    this.updateVirtualScroll();
  ...
  collapseAllGroups(): void {
    ...
    data.forEach(row => {
      if (row['_isGroupHeader']) {
        this.dataManager.collapseGroup(row['_groupKey'] as string);
      }
    });
    this.updateVirtualScroll();
    this.render();
  ...
  private render(): void {
    ...
    for (let i = visibleRange.startIndex; i <= visibleRange.endIndex; i++) {
      ...
      const groupKey = row['_groupKey'] as string;
  ...
  private injectEventHandlers(): void {
    ...
    groupToggles.forEach(toggle => {
      toggle.addEventListener('click', (e) => {
        e.stopPropagation();
        const groupKey = (toggle as HTMLElement).dataset.groupKey;
        if (groupKey) {
